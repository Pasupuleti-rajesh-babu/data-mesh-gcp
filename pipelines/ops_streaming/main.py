...
def run(project_id, subscription, raw_bucket_path, curated_bucket_path, pipeline_args):
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        _ = (
            ...
            | "WriteRawToGCS"
            >> beam.io.WriteToAvro(
                f"gs://{raw_bucket_path}/operations/raw_events",
        ...
        def format_for_parquet(element):
            return {
                ...
                "event_timestamp": float(element["event_timestamp"]),
                "service_name": element["service_name"],
                "status": element["status"],
                "payload": json.dumps(element["payload"]),
            }
        ...
        _ = (
            ...
            >> beam.io.WriteToParquet(
                f"gs://{curated_bucket_path}/operations/",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--raw_bucket_path", required=True, help="GCS path for raw data"
    )
    parser.add_argument(
        "--curated_bucket_path", required=True, help="GCS path for curated data"
    )
